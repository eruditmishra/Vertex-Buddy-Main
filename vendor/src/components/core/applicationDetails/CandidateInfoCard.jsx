...
const CandidateInfoCard = ({ candidateDetails }) => {
  const [showDropdown, setShowDropdown] = useState(false);

  return (
    <div
      className="bg-white flex flex-col gap-4 h-fit rounded-xl py-4 px-4 relative"
    ...
    >
      ...
      <div className=" flex flex-col gap-1 ">
        <h2 className=" text-[1.8rem] font-medium flex items-center gap-6 ">
          {candidateDetails?.firstName} {candidateDetails?.middleName}{" "}
        ...
        <p className=" text-[1.1rem] font-medium text-offset-black">
          {candidateDetails?.currentDesignation}
        </p>
      </div>
      {showDropdown && (
